...
Ko5hian is generated by gradle. Declare Views to generate Ko5hian in build.gradle.
...
This works well. Except that nested `it` is shadowed. Nobody have trouble by
shadowed `it`. But compiler warns noisily.


### Wrong Solution2

Extend Views to make it have the type of LayoutParams.
```kotlin
...
Use `typealias`.
```kotlin
typealias TextViewOn<P> = TextView

inline val TextViewOn<FrameLayout>.lParams: FrameLayout.LayoutParams
   get() = layoutParams as FrameLayout.LayoutParams

inline fun FrameLayout.textView
      (initializer: TextViewOn<FrameLayout>.() -> Unit): TextViewOn<FrameLayout>
{
   val view = TextView(context)
   view.layoutParams = FrameLayout.LayoutParams(WRAP_CONTENT, WRAP_CONTENT)
   view.initializer()
...
fun createView() {
   frameLayout(context) {
      textView {
         // here, this is TextViewOn<FrameLayout>

         // lParams is FrameLayout.LayoutParams, wow!
         lParams.width = WRAP_CONTENT

         // this is TextViewOn<FrameLayout>, so this is TextView.
         // We can call setText naturally.
         text = "Hello, Ko5hian!"
      }
   }
}
...
Ko5hian returns `ViewOn<ParentView>`.

```kotlin
class YourActivity : Activity() {
   private lateinit var rootLayout: FrameLayout
   private lateinit var button: ButtonOn<FrameLayout>
   ...
   private fun onButtonClick() {
      // get parent without `as`
      val rootLayout: FrameLayout = button.parentView

      // Of course lParams is FrameLayout.LayoutParams
      button.lParams {
         it.topMargin = dip(16)
      }
   }
   ...
   private fun createView() {
      rootLayout = frameLayout(this) {
         button = button {
            lParams.width  = WRAP_CONTENT
            lParams.height = WRAP_CONTENT
            text = "OK"
            onClick = { onButtonClick() }
         }

         // ImageView<FrameLayout> is a typealias,
         // so it can be stored into ImageView.
         imageView = imageView {
            lParams.width  = WRAP_CONTENT
            lParams.height = WRAP_CONTENT
         }
